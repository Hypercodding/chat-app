...
export const remove = mutation({
    ...
    if(!membership || membership.length !== 2){
        throw new ConvexError("This cconversation does not have any member")
    }
    ...
    if(!friendships){
        throw new ConvexError("Friendship not foend")
    }

    const message = await ctx.db.query("messages").withIndex("by_conversationId", q=>q.eq("conversationId", args.conversationId)).collect()
